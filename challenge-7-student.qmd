...
replace_unlikely_values <- function(measurements, min_value, max_value) {
  ...
  measurements <- ifelse(measurements < min_value | measurements > max_value, NA, measurements)
...
condition_index <- function(weight, length) {
  ...
  if (any(weight <= 0, length <= 0)) {
    stop("Weight and length must be positive values.")
  }
  
  index <- (weight / (length^3)) * 100000
  return(index)
}
...
fish |> 
  ...
  ggplot(aes(x = year, y = condition, color = species)) + 
